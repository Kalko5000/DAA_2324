...
void Constant::Adv(std::vector<int>& registros, std::string op) {
  std::cerr << "No se puede hacer Adv con una constante" << std::endl;
  exit(1);
}
...
void Registro::Adv(std::vector<int>& registros, std::string op) {
  int result{0};
  for (int i{0}; i < 3; i++) {
    result += registros[std::stoi(op) + i];
  }
  registros[0] = result;
}
...
void RegistroContenido::Adv(std::vector<int>& registros, std::string op) {
  op.erase(0, 1);
  int result{0};
  for (int i{0}; i < 3; i++) {
    result += registros[registros[std::stoi(op)] + 1];
  }
  registros[0] = result;
}